...
const SeatSelection: React.FC<SeatSelectionProps> = ({
    ...
    const filteredSeats = selectedSeats.filter(
        seat => currentSection === 'all' || seat.sectionKey === getSectionType(currentSection)
    );
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                        <div>
                            ...
                            <p className="opacity-70">
                                Choose {passengerCount} {passengerCount === 1 ? 'seat' : 'seats'} to
                                continue
                            </p>
                        </div>
                    ...
                    <TabsList className="w-full h-full grid grid-cols-5">
                        {aircraftSections.map(section => (
                            ...
                            >
                                <div
                                    className={
                                        'h-full w-full flex flex-col md:flex-row items-center justify-center gap-2'
                                    }
                                >
                    ...
                    {filteredSeats.length === 0 ? (
                        <div className="text-center p-4">
                            <p>
                                {currentSection === 'all'
                                    ? 'No Seats Selected'
                                    : `No ${getSectionType(currentSection)
                                          .replace('_', ' ')
                                          .toLowerCase()} seats selected.`}
                            </p>
                            <p className="text-sm opacity-70">
                                {selectedSeats.length}{' '}
                                {selectedSeats.length === 1 ? 'seat' : 'seats'} selected in other
                                sections
                            </p>
                        </div>
                    ) : (
                        filteredSeats.map(seat => (
                            <SeatCard key={seat.publicId} seat={seat} onRemove={onSeatRemove} />
                        ))
                    )}
                </CardContent>
            </Card>
...
const SeatCard: React.FC<SeatCardProps> = ({ seat, onRemove }) => {
    ...
    return (
        <div className="flex items-center justify-between p-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg group hover:shadow-md transition-all duration-200">
            <div className="flex items-center space-x-4">
                ...
                <div className="flex items-center justify-center w-12 h-12 rounded-md shadow-sm bg-gray-50 dark:bg-gray-700">
                    <svg viewBox="0 0 40 40" className="w-10 h-10">
                        <rect
                            ...
                            rx="3"
                            fill={
                                seat.sectionKey === 'FIRST_CLASS'
                                    ? '#c4b5fd'
                                    : seat.sectionKey === 'BUSINESS_CLASS'
                                    ? '#fcd34d'
                                    : seat.sectionKey === 'PREMIUM_ECONOMY'
                                    ? '#fb923c'
                                    : '#bfdbfe'
                            }
                            stroke={
                ...
                <div className="md:w-65 flex flex-col justify-between gap-2">
                    <div className="w-full flex items-center justify-between">
                        ...
                        <Badge
                            className={`md:w-40 text-[12px] md:text-sm px-2 py-0 ${getBadgeVariant()}`}
                        >
                            {formatSeatType(seat.sectionKey)}
                        </Badge>
                    </div>
            ...
            {/* Remove button */}
            <button
                onClick={() => onRemove(seat.publicId)}
